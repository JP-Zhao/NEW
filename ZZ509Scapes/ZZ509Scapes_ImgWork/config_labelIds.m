function config_labelIds(file_path_json, file_path_labelIds_png)
...
%图片保存的位置
json_path_list = dir(strcat(file_path_json,'*.json'));%获取该文件夹中所有json格式的图像
num = length(json_path_list);
name = {json_path_list.name};
% road的id是7
% sky的id是23
road = 7; sky = 23; rail = 10; unlabel = 0;
for i = 1 : num
    path = [file_path_json, cell2mat(name(i))];
    jsonData = loadjson(path); % jsonData是个struct结构
    %     jsonData.objects{1, 1}.polygon               %看看这个数据长什么样
    TempImg = uint8(zeros(jsonData.imageHeight, jsonData.imageWidth));          %背景是unlabel = 0
    ...
    imwrite(TempImg, strcat(file_path_labelIds_png, erase(cell2mat(name(i)), '_gtFine_polygons.json'), '_gtFine_labelIds', '.png'));
