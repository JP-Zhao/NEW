...
#ifndef _TOP_fuct_SobOvlyIAem_H_
#define _TOP_fuct_SobOvlyIAem_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "TOP_fuct_SobOvlyIAem_memcore.h"

SC_MODULE(TOP_fuct_SobOvlyIAem) {
...
protected:
    ...
    // buffer instances
    TOP_fuct_SobOvlyIAem_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(TOP_fuct_SobOvlyIAem) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "TOP_fuct_SobOvlyIAem_memcore_U_";
            name += szTmp;

            buffer[i] = new TOP_fuct_SobOvlyIAem_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "TOP_fuct_SobOvlyIAem.trace");
            // ports
